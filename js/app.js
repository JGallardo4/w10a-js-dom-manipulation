...
tweets.forEach((t) => {
    var tweets_section = document.getElementById('tweets');

    var tweet = document.createElement('article');
    tweet.className = 'tweet';
    tweet.setAttribute('id', `${t.username.toLowerCase()}-tweet`);

    var tweet_content = document.createElement('div');
    tweet_content.innerHTML = t.tweet_content;
    tweet_content.className = 'tweet__content';
...
// 1. querySelector()
var brent_tweet = document.querySelector("#brent-tweet");

brent_tweet.querySelector(".tweet__name").style.fontWeight = "bold";

// 2. querySelectorAll()
var usernames = Array.from(document.querySelectorAll(".tweet__username"));
...
var all_tweet_contents = Array.from(document.getElementsByClassName("tweet__content"));
all_tweet_contents.forEach((t) => {
    t.insertAdjacentText("afterbegin", '"');
    t.insertAdjacentText("beforeEnd", '"');
});

// 5. getElementByTagName()
var icons = Array.from(document.getElementsByTagName("i"));
icons.forEach((i) => {
    i.style.color = "white";
...
// 7. remove()
// Remove all but the first icon
icons.shift();
icons.forEach((i) => { i.remove() });

// 8. classList.add()
var first_tweet = Array.from(document.getElementsByClassName("tweet"))[0];
first_tweet.classList.add("first-tweet");

// 9. classList.remove()
first_tweet.classList.remove("first-tweet");

// 10. classList.toggle()
first_tweet.addEventListener("click", function() {
    this.classList.toggle("highlight-tweet");
});